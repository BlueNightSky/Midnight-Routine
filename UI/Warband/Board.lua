...
function MR:RefreshWarbandBoard(reuseData)
    ...
    if frame.moduleSettingsBtn then
        WBStylePillButton(frame.moduleSettingsBtn, false)
    end
    ...
    for index, entry in ipairs(listData) do
        ...
        hideBtn._label:SetFont(ns.FONT_HEADERS, 10, GetFontFlags())
    ...
    if concentrationEntries and #concentrationEntries > 0 and frame.concentrationPane then
        ...
        for index, concentrationEntry in ipairs(concentrationEntries) do
            ...
            chip._label:SetFont(ns.FONT_ROWS, math.max(8, GetFontSize() - 1), GetFontFlags())
            chip._label:SetText(labelText)
            chip._label:SetTextColor(0.84, 0.89, 0.96)
            chip._value:SetFont(ns.FONT_HEADERS, math.max(9, GetFontSize() - 1), GetFontFlags())
    ...
    for _, moduleEntry in ipairs(selected.modules) do
        ...
        if hideCompletedRows and #visibleRows == 0 then
            ...
            card._arrow:SetFont(ns.FONT_HEADERS, math.max(10, GetFontSize() + 1), GetFontFlags())
            card._arrow:SetText(isCollapsed and "+" or "-")
            card._arrow:SetTextColor(0.62, 0.72, 0.78)
            card._title:SetFont(ns.FONT_HEADERS, math.max(10, GetFontSize() + 1), GetFontFlags())
            card._title:SetText(moduleEntry.label)
            card._title:SetTextColor(math.min(1, mr + 0.08), math.min(1, mg + 0.08), math.min(1, mb + 0.08))
            card._progress:SetFont(ns.FONT_ROWS, math.max(8, GetFontSize() - 1), GetFontFlags())
...
function MR:ToggleWarbandBoard()
    ...
    if not self.altBoardFrame then
        ...
        ns.RegisterThemedFontString(title, 0.90, 0.95, 1.0)

        local summaryValue = titleBar:CreateFontString(nil, "OVERLAY")
        summaryValue:SetFont(ns.FONT_ROWS, math.max(8, GetFontSize() - 1), GetFontFlags())
        ...
        summarySub:SetFont(ns.FONT_ROWS, math.max(8, GetFontSize() - 1), GetFontFlags())
        ...
        leftLabel:SetFont(ns.FONT_ROWS, math.max(9, GetFontSize()), GetFontFlags())
        leftLabel:SetPoint("TOPLEFT", leftPane, "TOPLEFT", 12, -12)
        leftLabel:SetText(L["AltBoard_Characters"] or "Characters")
        ns.RegisterThemedFontString(leftLabel, 0.70, 0.82, 0.86)
        ...
        showHiddenLabel:SetFont(ns.FONT_ROWS, 9, GetFontFlags())
        ...
        ns.RegisterThemedFontString(showHiddenLabel, 0.70, 0.88, 0.85)
        ...
        searchIcon:SetFont(ns.FONT_ROWS, math.max(9, GetFontSize() - 1), GetFontFlags())
        searchIcon:SetPoint("LEFT", searchBox, "LEFT", 8, 0)
        searchIcon:SetText(">")
        ns.RegisterThemedFontString(searchIcon, 0.42, 0.52, 0.60)

        local searchPlaceholder = searchBox:CreateFontString(nil, "OVERLAY")
        searchPlaceholder:SetFont(ns.FONT_ROWS, math.max(9, GetFontSize() - 1), GetFontFlags())
        ...
        clearSearchLabel:SetFont(ns.FONT_HEADERS, 9, GetFontFlags())
        clearSearchLabel:SetPoint("CENTER", clearSearchBtn, "CENTER", 0, 1)
        clearSearchLabel:SetText("x")
        ns.RegisterThemedFontString(clearSearchLabel, 0.58, 0.68, 0.76)
        clearSearchBtn:Hide()
        ...
        clearSearchBtn:SetScript("OnLeave", function(selfBtn)
            ...
            local r, g, b = ns.ResolveThemeColor(0.58, 0.68, 0.76)
            clearSearchLabel:SetTextColor(r, g, b)
        end)
        ...
        local function CreateAltBoardTab(label, viewKey)
            ...
            lbl:SetFont(ns.FONT_ROWS, math.max(8, GetFontSize() - 1), GetFontFlags())
        ...
        concentrationTrackerLabel:SetFont(ns.FONT_ROWS, 9, GetFontFlags())
        ...
        ns.RegisterThemedFontString(concentrationTrackerLabel, 0.70, 0.88, 0.85)
        ...
        ns.RegisterThemedTexture(concentrationAccent, 0.50, 0.42, 0.72, 0.95)
        ...
        ns.RegisterThemedFontString(concentrationTitle, 0.78, 0.76, 0.92)
        ...
        hideCompletedLabel:SetFont(ns.FONT_ROWS, 9, GetFontFlags())
        ...
        ns.RegisterThemedFontString(hideCompletedLabel, 0.70, 0.88, 0.85)
        ...
        detailEmptyLabel:SetFont(ns.FONT_ROWS, math.max(9, GetFontSize()), GetFontFlags())
        ...
        overviewEmptyLabel:SetFont(ns.FONT_ROWS, math.max(9, GetFontSize()), GetFontFlags())
        ...
        ns.RegisterThemedTexture(moduleAccent, 0.16, 0.78, 0.68, 0.92)
        ...
        ns.RegisterThemedFontString(moduleScope, 0.55, 0.74, 0.75)
        ...
        ns.RegisterThemedFontString(moduleSettingsBtn._label, 0.62, 0.76, 0.78)
